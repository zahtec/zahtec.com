...
export default function Tag({ fullName, name, fontaw, color, noMargin }: { fullName: string; name: string; fontaw?: string; color: string[]; noMargin: boolean }) {
    return (
        <div id={name} className={`flex justify-center items-center select-none w-40 min-w-40 h-16 p4 snap-center rounded-lg transition-tag bg-gradient-140 border-solid border-xs ${color[0]} ${color[1]} ${color[2]}${noMargin ? '' : ' first-of-type:ml-12 last-of-type:mr-12'}`}>
