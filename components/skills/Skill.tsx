...
export default function Skill({ fullName, name, desc, fontaw, color, fontawPrefix = 'brands', hasProjects = true }: StoredSkill) {
    ...
    return (
        <div className={`load-anim flex w-full items-center rounded-xl justify-center flex-col mb-8 py-8 px-4 relative text-center transition-filter duration-300 overflow-hidden drop-shadow-gr bg-gradient-140 border-solid border-xs ${color[0]} ${color[1]} ${color[2]} hover:drop-shadow-gr-hover`}>
